...
from transformer_autoencoder.dataset import ProtSeqDatasetBert, SyntheticDataset, ShuffleDataset, padding_collate_fn
...
def get_sequence_loaders(batch_size=32, vocab_size=8000, dataset_dir='/data/uniparc', buffer_size=int(3e7), no_verification=False):
    train_dataset = ProtSeqDataset(mode='train', vocab_size=vocab_size, no_verification=no_verification, dataset_dir=dataset_dir)
    val_dataset = ProtSeqDataset(mode='val', vocab_size=vocab_size, dataset_dir=dataset_dir)
    test_dataset = ProtSeqDataset(mode='test', vocab_size=vocab_size, dataset_dir=dataset_dir)
